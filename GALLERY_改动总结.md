...
7. **修复 Web 页图标 404**：`content/web` 目录大写残留导致的 Lektor 构建缓存冲突。
...
- **可选 url**：`url` 字段必须用 `type = string` 才能为空（`type = url` 时访问空值会报 `Missing URL` 错误）。
...
| 文件 | 说明 |
|------|------|
| `models/gallery.ini` | Gallery 模型：含 `name`、`cover_image`、`columns` 字段；`[children] model = gallery-item` |
| `models/gallery-item.ini` | 叶子条目模型：`name`、`url`（string 可选）、`cover_image`、`description` |
...
| `content/gallery/contents.lr` | 页面入口，`_model: gallery`，`columns: 5` |
...
| 文件/位置 | 说明 |
|------|------|
| `content/Web` → `content/web` | 顶层目录改小写 |
...
- **叶子跳外链**：子项 `contents.lr` 写 `url: https://...`；不写则跳详情页。
- **嵌套子目录**：子项 `contents.lr` 写 `_model: gallery` 即可继续嵌套。
